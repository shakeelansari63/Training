...
def random_expense_category():
    return random.choice(["Rent", "Groceries", "Utilities", "Entertainment", "Transportation", "Healthcare"])
...
def generate_expense(expense_date):
    ...
    return {
        ...
        "Comment": random.choice(["Grocery shopping", "Movie night", "Utility bill", "Transport cost", "Doctor visit"])
    }

def generate_expense_category(start_date):
    category = random_expense_category()
    num_expenses = random.randint(3, 6)
    expenses = [generate_expense(start_date + timedelta(days=random.randint(0, 27))) for _ in range(num_expenses)]
...
def generate_monthly_budgets(start_month: int, start_year: int, end_month: int, end_year: int):
    budgets = []
...
store_state = {
    "budget": {
        "activeBudget": budgets[-1],
        "pastBudgets": budgets[:-1]
    },
    "setting": {
        "currency": "USD",
        "theme": "dark"
