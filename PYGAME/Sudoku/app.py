        ...
        for event in pygame.event.get():
            ...
            ## Used Pressed a Key
                ...
                if event.key in self.num_keys and self.selected_cell is not None:
                    self.playing_grid[self.selected_cell[1]][self.selected_cell[0]] = int(event.unicode)
...
##########################################################################################
    ...
    def check_duplicate_in_columns(self):
        for i in range(9):
            non_zero_column = [ self.playing_grid[j][i] for j in range(9) if self.playing_grid[j][i] != 0]
...
##########################################################################################
    ...
    def draw_grid(self):
        ...
        pygame.draw.rect(self.screen, BLACK, (GAME_AREA["START-X"], GAME_AREA["START-Y"], WIDTH - 40, HEIGHT - 120), 3)
    ...
    def mouse_on_grid(self):
        if self.mouse_pos[0] > GAME_AREA["START-X"] and self.mouse_pos[0] < GAME_AREA["END-X"] \
            ...
            self.selected_cell = (cell_row, cell_col) if (cell_row, cell_col) not in self.locked_cells else None
        else:
            self.selected_cell = None
